#!/usr/bin/env bash
# Agent PreToolUse guard (matcher: Bash) -- blocks clearly-destructive commands.
# High-signal only, to avoid nagging. exit 2 blocks the tool; stderr is shown to the agent.
...
block() { echo "🛑 Blocked by agent guard: $1 -- run it yourself if you're sure." >&2; exit 2; }
